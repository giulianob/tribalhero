...
namespace Game.Logic.Procedures {
    public partial class Procedure  {
        ...
        private static bool remove_from_normal(TroopStub source, TroopStub target) {
            foreach (KeyValuePair<FormationType, Formation> kvp in (IEnumerable<KeyValuePair<FormationType, Formation>>)target) {
                foreach (KeyValuePair<ushort, ushort> unit in kvp.Value) {
                    ushort count;
                    if (!source[FormationType.Normal].TryGetValue(unit.Key, out count)) return false;
            ...
            foreach (KeyValuePair<FormationType, Formation> kvp in (IEnumerable<KeyValuePair<FormationType, Formation>>)target) {
                foreach (KeyValuePair<ushort, ushort> unit in kvp.Value) {
                    if (source[FormationType.Normal].remove(unit.Key, unit.Value) != unit.Value) return false;
                }
            }
            source.EndUpdate();

...
namespace Game.Setup {
    ...
    public class ActionFactory {
        static Dictionary<int, ActionRecord> dict;
            ...
            using (CSVReader reader = new CSVReader(new StreamReader(new FileStream(filename, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)))) {
                ...
                while ((toks = reader.ReadRow()) != null) {
                    ...
                    if ((action_index = byte.Parse(toks[col["Index"]])) == 0) {
                        ...
                        if (!uint.TryParse(toks[col["EffectReq"]], out action_req.effect_req_id)) action_req.effect_req_id = 0;
                        if (toks[col["EffectReqInherit"]].Length > 0) {
                            action_req.effect_req_inherit = (EffectInheritance)Enum.Parse(typeof(EffectInheritance), toks[col["EffectReqInherit"]], true);
                        }
                        else {

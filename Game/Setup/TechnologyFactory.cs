...
namespace Game.Setup {
    ...
    public class TechnologyFactory {
        static bool initc = false;

        static Dictionary<uint, TechnologyBase> technologies = new Dictionary<uint, TechnologyBase>();
        ...
        public static void init(string technology_filename, string technology_effects_filename) {
            ...
            using (CSVReader reader = new CSVReader(new StreamReader(new FileStream(technology_filename, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)))) {
                ...
                while ((toks = reader.ReadRow()) != null) {
                    ...
                    tech.resources = new Resource(int.Parse(toks[col["Crop"]]), int.Parse(toks[col["Gold"]]), int.Parse(toks[col["Iron"]]), int.Parse(toks[col["Wood"]]), int.Parse(toks[col["Labor"]]));
            ...
            using (CSVReader reader = new CSVReader(new StreamReader(new FileStream(technology_effects_filename, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)))) {
                Dictionary<string, int> col = new Dictionary<string, int>();
                String[] toks;
                TechnologyBase tech;
                for (int i = 0; i < reader.Columns.Length; ++i) {

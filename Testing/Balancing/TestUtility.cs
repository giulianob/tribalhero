...
namespace Testing.Balancing {
    public class TestUtility {
        ...
        public static void TestUnitPerUpkeep(UnitType type, UnitType target, double multiplier, double expectAdvantage, double errorMargin=.15) {
            ...
            Assert.IsTrue(expectAdvantage - errorMargin < actualAdvantage && actualAdvantage < expectAdvantage + errorMargin,
                                        "Def[{0}/{6}] Atk[{1}/{7}]'s actual advantage[{2}] is not close to expected [{3}] Def[{4}] Atk[{5}]",
                                                    defender.Upkeep()/typeUpkeep,
                                                    attacker.Upkeep()/targetUpkeep,
                                                    actualAdvantage,
                                                    expectAdvantage,
                                                    expectAdvantage > 0 ? expectAdvantage * ((double)count / typeUpkeep) : 0,
                                                    expectAdvantage <= 0 ? -1 * expectAdvantage * ((double)count / targetUpkeep) : 0,
                                                    ((double)count / typeUpkeep),
                                                    ((double)count / targetUpkeep));
        }

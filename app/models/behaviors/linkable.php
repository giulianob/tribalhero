 ...
 *
 * @version 1.0.1:
 * -empty fields will automatically choose all fields now
 * -does not need to specify the foreign key condition if you use the conditions array
 * -added count() support
 */
...
class LinkableBehavior extends ModelBehavior {
	...
	public function beforeFind(&$Model, $query) {
		if (isset($query[$this->_key])) {
			$skipFields = isset($query['fields']) && !is_array($query['fields']) && strpos($query['fields'], 'COUNT(*)') !== FALSE;

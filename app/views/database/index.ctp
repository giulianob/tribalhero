	...
	<div class="span-20 last">
		...
		<div class="span-4 prepend-top<?php echo ($i == 0 ? ' last' : ''); ?>">
			<div class="span-4 last" style="height: 40px">
				<?php echo $this->Html->link($this->Html->image('db/units/' . $info['sprite'] . '.png', array('class' => 'center-block')), array('action' => 'view', $key), array('class' => 'center-block', 'escape' => false)); 
				?>
			</div>
			<div class="span-4 last text-center"><strong><?php echo $this->Html->link($info['name'], array('action' => 'view', $key)); ?></strong></div>
		</div>
...
<div class="span-20 last prepend-top append-bottom">
	...
	<div class="span-20 last">
		...
		$i = 0;
		$f = true;
		foreach ($technologies as $key => $info) :
			if ($i % 5 == 0) echo (!$f ? '</div>' : '') . '<div class="span-20 last '. (!$f ? 'prepend-top' : '') .'">';
			$f = false;
			$i = ($i + 1) % 5;
		?>
		<div class="span-4 prepend-top<?php echo ($i == 0 ? ' last' : ''); ?>">
			<div class="span-4 last" style="height: 40px">
				<?php echo $this->Html->link($this->Html->image('db/icons/props/paper-scroll.png', array('class' => 'center-block')), array('action' => 'view', $key), array('escape' => false)); 
				?>
			</div>		
			<div class="span-4 last text-center"><strong><?php echo $this->Html->link($info['name'], array('action' => 'view', $key)); ?></strong></div>
		</div>

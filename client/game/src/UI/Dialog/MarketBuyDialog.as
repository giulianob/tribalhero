package src.UI.Dialog
{
    ...
    public class MarketBuyDialog extends GameJPanel
	{
		...
		public function MarketBuyDialog(structure:StructureObject, wood:int, crop:int, iron:int, onAccept:Function):void
		{
			...
			lstResourceType.setListData(Formula.resourcesBuyable(structure.level));
		...
		public function onResourceChange(e:Event):void
		{
			...
			var maxAffordable:int = int(city.resources.gold.getValue() / pricePerResource) * Formula.RESOURCE_CHUNK;
			
			var value:int = sldAmount.getValue();
			
			var min:int = Math.min(Formula.RESOURCE_CHUNK, maxAffordable);
			var max:int = Math.min(Formula.resourceMaxTrade(structure.level), maxAffordable);
		...
		public function onAmountChange(e: *):void
		{
			...
			lblTradePrice.setText(Formula.marketBuyCost(pricePerResource, sldAmount.getValue()).toString());
		}

...
public class MarketBuyDialog extends GameJPanel
{
	...
	public function MarketBuyDialog(structure: StructureObject, wood: int, crop: int, iron: int, onAccept: Function):void
	{				
		createUI();
		setIcons();
	...
	public function onResourceChange(e: Event):void {
		...
		var maxAffordable: int = int(city.resources.gold.getValue()/(pricePerResource * (1.0 + Formula.marketTax(structure)))) * Formula.RESOURCE_CHUNK;
		
		var value: int = sldAmount.getValue();
		
		var min: int = Math.min(Formula.RESOURCE_CHUNK, maxAffordable);
		var max: int = Math.min(Formula.RESOURCE_MAX_TRADE, maxAffordable);
	...
	public function onAmountChange(e: Event):void {
		...
		lblTax.setText("* " + int(tax * 100).toString() + "% tax included");
		...
		lblTradePrice.setText(Formula.marketBuyCost(pricePerResource, sldAmount.getValue(), tax).toString() + "*");				
	}	
	...
	public function createUI(): void {
		...
		lstResourceType.setSize(new IntDimension(40, 21));

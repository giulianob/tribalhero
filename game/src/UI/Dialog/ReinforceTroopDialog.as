...
public class ReinforceTroopDialog extends GameJPanel {
	...
	{					
		...
		var localTilelists: Array = TroopGridList.getGridList(srcTroop, city.template, [Formation.Normal]);
			
		pnlLocal.append(TroopGridList.stackGridLists(localTilelists));
		...
		attackTilelists = TroopGridList.getGridList(newTroop, city.template, destFormations);
		
		pnlAttack.append(TroopGridList.stackGridLists(attackTilelists));
	...
	public function getTroop(): Troop
	{
		...
		{
			newTroop.add((attackTilelists[i] as TroopGridList).getFormation());
		}
